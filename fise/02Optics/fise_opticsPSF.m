...
% This script shows how to use shift-invariant optics with Gaussian, Lorentzian 
% and Pillbox point spread functions (PSFs).
% 
% The script calculates the impact of these PSFs on a simple grid scene, visualizing 
% the outcome in an optical image window.
% 
% PSFs are generated using _`siSynthetic`_, which supports various types and 
% introduces wavelength-dependent chromatic aberrations. Read the siSynthetic 
% function to learn how to create optics using a defined PSF.  The utility _ieSaveSIDataFile_ 
% may also be helpful.
% 
...
% * Run:  fise_setHTMLImageSize('fise_opticsPSF.html', 'fise_opticsPSF.html', 
% 600)
...
% The pillbox was often used in the past because it can be computed very quickly.  
% It isn't good for much, but I stuck it in here anyway.  Mostly, notice how much 
% blurrier the pillbox is than an Airy pattern with the same disk size.
...
% The Lorentzian gamma parameter seems to run nicely from 1 to 10 or so on this 
% support. Log spacing converts better to spread than linear spacing.
...
% This example makes a Gaussian that is narrow in one direction and broad in 
% the other. This orientation difference is called _astigmatism_.
...
% You can see the wavelength-dependent blur in this rendition with space on 
% one axis and wavelength on the other. The spread of the light is smaller at 
% the short wavelengths and increases at the long wavelengths.
